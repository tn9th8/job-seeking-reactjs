...
import { callFetchSkillList } from './api';
interface Item {
    name: string;
    value: string;
}
export const fetchListSkill = async () => {
    const res = await callFetchSkillList();
    if (res.data) {
        return res.data;
    }
}

export const changeEnumListSkill = (data: { name: string, value: string }[]) => {
    const convertedObject: Record<string, string> = data.reduce(
        (acc, item) => {
...
// export const SKILLS_LIST: { label: string, value: string }[] =
//     [
